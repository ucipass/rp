...
module.exports = function(){
    ...
    connection.getClient = async (client)=>{
        ...
        let reply = await Client.findOne({ name : client}).lean(true)  
        if (reply && reply.length) {
            delete reply._id
            delete reply.__v
            return reply
        }  
        log.debug("getClient record found:", reply ? true : false) 
        return reply 
    }

    connection.getClients = async (client)=>{
        let reply = await Client.find({}).lean(true)
        reply = reply.map(json => {
            delete json._id
            delete json.__v
            return json
        })          
        log.debug("getClients record found:", reply.length)
        return reply         
    }
    ...
    connection.deleteClientsAll = async ()=>{
    ...
    connection.deleteRoomsAll = async ()=>{
