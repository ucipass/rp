<template>
  <b-container fluid v-if='showMainWebclients'>
    <h1>Web Clients</h1>
...
import {URL_WEBCLIENTS_READ, URL_WEBCLIENTS_DELETE } from './constants.js';
...
export default {
  ...
  methods:{
    ...
    deleteData: async function (index){
      ...
      console.log(URL_WEBCLIENTS_DELETE)
      let response = await axios
      .post(URL_WEBCLIENTS_DELETE,this.receivedData[index])
    ...
    async refresh(){
      axios
      .post(URL_WEBCLIENTS_READ,{})
      .then(response => {
        console.log("URL_WEBCLIENTS_READ:",response.data)
      ...
      console.log("Event: showMainWebclients")      
    },
  ...
  mounted: async function () {
    this.refresh();
    eventBus.$on('showMainWebclients', () => {
      hideMainAll()
      this.refresh();
      this.showMainWebclients = true;
      console.log("Event: showMainWebclients");
    })    
    eventBus.$on('hideMainWebclients', () => {
      this.showMainWebclients = false
      console.log("Event: hideMainWebclients")
    })       
